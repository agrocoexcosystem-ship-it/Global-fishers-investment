        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex lg:hidden items-center gap-4 relative z-50">
            <button 
                ...
                className="p-2.5 text-slate-600 bg-white/80 backdrop-blur-md border border-white/50 rounded-full shadow-sm hover:bg-emerald-50 hover:text-emerald-600 transition-all active:scale-90"
            >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-0 z-40 bg-white/98 backdrop-blur-xl pt-28 px-6 lg:hidden flex flex-col overflow-y-auto"
          >
             {/* Mobile Translate Placeholders - handled somewhat oddly by GTranslate script, making sure elements exist */}
             <div className="flex justify-center mb-8 bg-slate-50 p-3 rounded-2xl mx-auto w-fit">
            ...
            <div className="space-y-4">
              {navLinks.map((link, idx) => (
                ...
                >
                    <Link 
                    to={link.path} 
                    className={`block px-6 py-4 text-xl font-bold rounded-2xl transition-all ${location.pathname === link.path ? 'bg-emerald-50 text-emerald-600 shadow-inner border border-emerald-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
                    >
