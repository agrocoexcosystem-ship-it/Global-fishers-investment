...
create table if not exists transactions (
  ...
  created_at timestamp with time zone default timezone('utc'::text, now()),
  method text,
  details text -- For wallet addresses or notes
);
...
$$ language plpgsql security definer;
-- note: trigger on auth.users usually requires superuser, might fail via API if not allowed
`;
...
async function main() {
    ...
    try {
        ...
        // Attempting to use the 'query' endpoint which is often available for PG execution via API
        console.log("🛠️ Applying Database Schema (Tables, RLS, Triggers)...");
        try {
            ...
            console.error("⚠️ SQL Execution Failed via Management API.");
            console.error("   Reason:", sqlErr.response?.data?.error || sqlErr.message);
            console.log("   Trying fallback: Create Tables via Supabase Client (Limited support)...");
            // Fallback: We can't actually do CREATE TABLE via JS client easily without PG driver
            // Skipping fallback, rely on user to run SQL if this fails.
        }
        ...
        console.error("\n❌ ERROR:", e.message);
        if (e.response) console.error("   API Detail:", e.response.data);
    }
}
